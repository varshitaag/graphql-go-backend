...
func (h *Handler) execute(
	...
	for _, sel := range op.SelectionSet {
		...
		// Filter out unrequested fields from the value based on the selection set
		result[field.Alias] = filterFields(value, field.SelectionSet)
	}
...
// filterFields takes the resolved struct(s) and applies the GraphQL selection set.
// It maps the struct fields to a map[string]interface{} containing only requested fields.
func filterFields(data interface{}, selections ast.SelectionSet) interface{} {
	if data == nil {
		return nil
	}

	// Handle slice of Books
	if books, ok := data.([]models.Book); ok {
		var out []map[string]interface{}
		for _, b := range books {
			out = append(out, mapBook(b, selections))
		}
		return out
	}
    // Handle slice of pointers to Books (just in case)
	if books, ok := data.([]*models.Book); ok {
		var out []map[string]interface{}
		for _, b := range books {
            if b != nil {
	...
	// Handle single Book pointer
	if book, ok := data.(*models.Book); ok {
		if book == nil {
			return nil
		}
		return mapBook(*book, selections)
	}

	// Handle single Book struct
	if book, ok := data.(models.Book); ok {
		return mapBook(book, selections)
	}
...
func mapBook(b models.Book, selections ast.SelectionSet) map[string]interface{} {
	...
	for _, sel := range selections {
		...
		switch field.Name {
		case "id":
			out[field.Alias] = b.ID
		case "title":
			out[field.Alias] = b.Title
		case "author":
			out[field.Alias] = b.Author
		case "year":
			out[field.Alias] = b.Year
		case "price":
			out[field.Alias] = b.Price
		case "createdAt":
			out[field.Alias] = b.CreatedAt
		case "updatedAt":
			out[field.Alias] = b.UpdatedAt
		case "__typename":
			out[field.Alias] = "Book"
		}
	}
	return out
}
